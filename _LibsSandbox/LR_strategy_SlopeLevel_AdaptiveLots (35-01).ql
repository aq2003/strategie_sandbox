...
// Derives from 35-1
// LRSLAL_long_CalcNextTSSlope and LRSLAL_short_CalcNextTSSlope functions to calculate nextTS and slope as on serving a position
// LRSLAL_long_Calc1NextTSSlope and LRSLAL_short_Calc1NextTSSlope functions to calculate nextTS and slope on starting a position
...
OBV(period) :=
{
	...
	// +++ Debug
	//log("OBV_finished;period=" + period + ";OVB=;" + result + ";vol_sum=;" + vol_sum + ";OVBP=;" + (100% * result / vol_sum));
	// ---
	
	result = (100% * result / vol_sum);
};
						...
						<< _long_con3 == true;
					{
						_long_con6 = (close[offset] > ind("LinearRegression", "high", "low", predict_window_support, "low", train_window_support)) << _long_con5 == true;
						{
							...
							*/
							...
							result = (_long_con8 & (OBV(train_window) < 0%))
						||
						...
						<< _short_con3 == true;
					{
						_short_con6 = (close[offset] < ind("LinearRegression", "low", "high", predict_window_resistance, "high", train_window_resistance)) << _short_con5 == true;
						{
							...
							*/
							...
							result = (_short_con8 & (OBV(train_window) > 0%))
						||
		...
		..{
			...
			*/
			...
			res = LRSLAL_long_CalcNextTSSlope(
					"high", 	// A line type of the indicator which serves as nextTSlong base
					"low",	// A price type of the indicator which serves as nextTSlong base
		...
		&&
			...
			*/
			...
			res = LRSLAL_short_CalcNextTSSlope(
					"low", 	// A line type of the indicator which serves as nextTSlong base
