...
) :=
{
	...
	(
		...
		|
		// Last close crossed up low predicted resistance line
		(
			/*con8 =*/ (close[offset] #^ ind("LinearRegression", "high", "low", predict_window_resistance, "low", train_window_resistance)[offset]
			& close[offset] > ind("LinearRegression", "high", "high", predict_window, high_offset, train_window)[offset])
		)
	);
...
) :=
{
	...
	(
		...
		|
		// Last close crossed down predicted high resistance line
		(
			/*con8 =*/ (close[offset] #_ ind("LinearRegression", "low", "high", predict_window_support, "high", train_window_support)[offset]
			& close[offset] < ind("LinearRegression", "low", "low", predict_window, low_offset, train_window)[offset])
		)
	);
