...
) :=
{
	...
	_long_con0 = ((time < expiration_time & (time >= _day_start_time & time < _day_end_time | time >= _night_start_time & time < _night_end_time) & account == 0l);
	{
		...
		||
			...
			{
				nextTSlong_index = find_min_price_index(train_window);
				result = (
					close[offset] > ind("LinearRegression", "high", "high", predict_window, high_offset, train_window)[offset]
					& close[offset] #^ ind("LinearRegression", "line", "low", "once", "low", candle.time[nextTSlong_index-2c], candle.time[-1c])
				) << _long_con7 != true
			||
				result = _long_con7 << _long_con7 == true
			}
		}
	||
	...
	_short_con0 = ((time < expiration_time & (time >= _day_start_time & time < _day_end_time | time >= _night_start_time & time < _night_end_time) & account == 0l);
	{
		...
		||			
			_short_con7 = (
				...
				& close[offset] #_ ind("LinearRegression", "low", "low", predict_window_resistance, "low", train_window_resistance)[offset]
			) << _short_con1 != true;
			
			{
				nextTSshort_index = find_max_price_index(train_window);
				result = (
					close[offset] < ind("LinearRegression", "low", "low", predict_window, low_offset, train_window)[offset]
					& close[offset] #_ ind("LinearRegression", "line", "high", "once", "high", candle.time[nextTSshort_index-2c], candle.time[-1c])
				) << _short_con7 != true
			||
				result = _short_con7 << _short_con7 == true
			}
		}
	||
...
{			
	...
	_slope_short = (
		ind("LinearRegression", "slope", "high", "once", "none", candle.time[nextTSshort_index - 2c], candle.time[-1c]) 
		/ high[nextTSshort_index - 2c]
		* 100%
	);
	
	{
		_slope_short = pslope_short_start << _slope_short > pslope_short_start;
		_nextTSshort += (1p * _slope_short * (abs(nextTSshort_index) / 1c));
	...
	||
		...
		&&
			...
			my_slope_short = (
				...
				/ high[-train_window - 1c]
				* 100%
			);
			my_nextTSshort = (ind("LinearRegression", "low", "high", predict_window, high_offset, train_window)[-1c]);
			div_slope_short = (
				(my_nextTSshort - ind("LinearRegression", "low", "high", predict_window, low_offset, train_window)[-2c]) 
				/ my_nextTSshort
				* 100%
			);
			{
