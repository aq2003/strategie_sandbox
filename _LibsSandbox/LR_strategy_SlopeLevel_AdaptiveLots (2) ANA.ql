		...
		
			{
				...
				
				{
					log("looking_for_closing_long") << account > 0l;
					{
						...
						/*||
							stop() << account > 0l
								& (time >= day_start_time & time < day_end_time | time >= night_start_time & time < night_end_time)
								& low[-1c] < (LRSL = ind("LinearRegression", "low", "low", predict_window, low_offset, train_window)[-1c])
							;
							log("long_lr_globalTS;pos.abs_profit=;" + pos.abs_profit + ";pos.age=;" + pos.age + ";LRSL=;" + LRSL + ";no_activity=;" + abs(no_activity)) << account == 0l
						*/||
							no_activity = no_activity_periods << account > 0l;
						current_slope = (ind("LinearRegression", "slope", "low", predict_window, high_offset, train_window)[-1c]);
						no_activity = 
						(
							(current_slope - slope_long_level) * (no_activity_periods - 1c) 
							/ (slope_short_level - slope_long_level) + 1c
						);
				...
				||
					...
					/*||
						stop() << account < 0l
							& (time >= day_start_time & time < day_end_time | time >= night_start_time & time < night_end_time)
							& high[-1c] > (LRSL = ind("LinearRegression", "high", "high", predict_window, high_offset, train_window)[-1c])
						;
						log("short_lr_globalTS;pos.abs_profit=;" + pos.abs_profit + ";pos.age=;" + pos.age + ";LRSL=;" + LRSL + ";no_activity=;" + abs(no_activity)) << account == 0l
					*/||
						no_activity = no_activity_periods << account < 0l;
						current_slope = (ind("LinearRegression", "slope", "high", predict_window, high_offset, train_window)[-1c]);
						no_activity = 
						(
							no_activity_periods - (current_slope - slope_long_level) * (no_activity_periods - 1c) 
							/ (slope_short_level - slope_long_level) + 1c
						);

						// Debug Section 2.06.2024
						log("short_lr_NAS;started_watching;no_activity_periods=;" + no_activity_periods + ";no_activity=;" 
							+ no_activity + ";current_slope=;" + current_slope);
						//
