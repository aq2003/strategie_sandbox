...
// 22.02.2026 19:19:00 LR_strategy_SlopeLevel_AdaptiveLots (35-5) ql script
// Created 22.02.2026 19:19:00
// con3 is modified - slope_high >= slope_long_leve | slope_low <= slope_short_level
...
) :=
{
				...
				& close[offset] > ind("LinearRegression", "high", "high", predict_window, high_offset, train_window)[offset]);
		{
				...
				) << _long_con1 == true;
			{
				_long_con3 = (
					...
					| ind("LinearRegression", "slope", "low", predict_window, low_offset, train_window)[-1c] <= slope_short_level
				) << _long_con2 == true;
	...
	*/
	...
	//result = /*con0 =*/ (time < expiration_time & (time >= _day_start_time & time < _day_end_time | time >= _night_start_time & time < _night_end_time));
	//result = (result & account == 0l);
	//
	//// Last close crossed up high predicted high line
	//result = (result & /*con1 =*/ (close[offset] #^ ind("LinearRegression", "high", "high", predict_window, high_offset, train_window)[offset]));
	//
	...
	//result = (result & /*con3 =*/ (ind("LinearRegression", "slope", "high", predict_window, high_offset, train_window)[-1c] > slope_long_level));
	//		
	//// Last close is higher than predicted high support
	//result = (result & /*con6 =*/ close[offset] > ind("LinearRegression", "high", "low", predict_window_support, "low", train_window_support));
	//	
...
) :=
{
	...
	*/
	...
	//result = (result & /*con1 =*/ (close[offset] #_ ind("LinearRegression", "low", "low", predict_window, low_offset, train_window)[offset]));
	//
	...
	//result = (result & /*con3 =*/ (ind("LinearRegression", "slope", "low", predict_window, high_offset, train_window)[-1c] < slope_short_level));
	//		
	//// Last close is lower than predicted low resistance
	//result = (result & /*con6 =*/ close[offset] < ind("LinearRegression", "low", "high", predict_window_resistance, "high", train_window_resistance));
	//		
...
) :=
{
				...
				
	{
		...
		..
		{
			...
			
			{
				...
				+ ";account=;" + account 
				+ ";equity=;" + equity 
				+ ";high=;" + ind("LinearRegression", "line", "high", predict_window_type, high_offset, train_window) 
				+ ";hhigh=;" + hhigh = ind("LinearRegression", "high", "high", predict_window_type, high_offset, train_window) 
				+ ";lhigh=;" + ind("LinearRegression", "low", "high", predict_window_type, high_offset, train_window) 
				+ ";high.slope=;" + ind("LinearRegression", "slope", "high", predict_window_type, high_offset, train_window) 
				+ ";high.mae=;" + ind("LinearRegression", "mae", "high", predict_window_type, high_offset, train_window) 
				+ ";low=;" + ind("LinearRegression", "line", "low", predict_window_type, low_offset, train_window)
				+ ";hlow=;" + ind("LinearRegression", "high", "low", predict_window_type, low_offset, train_window)
				+ ";llow=;" + llow = ind("LinearRegression", "low", "low", predict_window_type, low_offset, train_window)
				+ ";llow.slope=;" + ind("LinearRegression", "slope", "low", predict_window_type, low_offset, train_window)
				+ ";llow.mae=;" + ind("LinearRegression", "mae", "low", predict_window_type, low_offset, train_window)
				+ ";nextTSlong=;" + nextTSlong + ";slope_long_=;" + slope_long
				+ ";nextTSshort=;" + nextTSshort + ";slope_short=;" + slope_short
				+ ";absSLlong=;" + absSLlong
				+ ";absSLshort=;" + absSLshort
				+ ";train_window=;" + train_window
				+ ";train_window_resistance=;" + train_window_resistance + ";train_window_support=;" + train_window_support
				+ ";res_high=;" + ind("LinearRegression", "line", "high", predict_window_resistance, "high", train_window_resistance) 
				+ ";res_hhigh=;" + hhigh = ind("LinearRegression", "high", "high", predict_window_resistance, "high", train_window_resistance) 
				+ ";res_lhigh=;" + ind("LinearRegression", "low", "high", predict_window_resistance, "high", train_window_resistance) 
				+ ";res_high.slope=;" + ind("LinearRegression", "slope", "high", predict_window_resistance, "high", train_window_resistance) 
				+ ";res_high.mae=;" + ind("LinearRegression", "mae", "high", predict_window_resistance, "high", train_window_resistance) 
				+ ";sup_low=;" + ind("LinearRegression", "line", "low", predict_window_support, "low", train_window_support)
				+ ";sup_hlow=;" + ind("LinearRegression", "high", "low", predict_window_support, "low", train_window_support)
				+ ";sup_llow=;" + llow = ind("LinearRegression", "low", "low", predict_window_support, "low", train_window_support)
				+ ";sup_llow.slope=;" + ind("LinearRegression", "slope", "low", predict_window_support, "low", train_window_support)
				+ ";sup_llow.mae=;" + ind("LinearRegression", "mae", "low", predict_window_support, "low", train_window_support)
				+ ";channel_width=;" + (hhigh - llow)
				+ ";volume=;" + volume
				) 
				<< log.level != "Error";
				//step += 1n; 
	...
	||
		..{
			//--- 15.02.2026 --- {
			//--- 15.02.2026 --- 	// Debug
			//--- 15.02.2026 --- 	old_slope_long = slope_long << account > 0l;
			//--- 15.02.2026 --- 	debug_str_l = "debug_moving_nextTSlong";
			//--- 15.02.2026 --- 
			//--- 15.02.2026 --- 	//log(debug_str_l + ";started...");
			//--- 15.02.2026 --- 
			//--- 15.02.2026 --- 	my_slope_long = ind("LinearRegression", "slope", "low", predict_window, low_offset, train_window)[-1c] << account > 0l;
			//--- 15.02.2026 --- 	my_nextTSlong = ind("LinearRegression", "high", "low", predict_window, low_offset, train_window)[-1c];
			//--- 15.02.2026 --- 	//div_slope_long = ((my_nextTSlong - ind("LinearRegression", "high", "low", predict_window, low_offset, train_window)[-2c]) / 1p);
			//--- 15.02.2026 --- 	{
			//--- 15.02.2026 --- 		slope_long = my_slope_long << my_slope_long > slope_long /*& my_slope_long > div_slope_long*/;
			//--- 15.02.2026 --- 		// Debug
			//--- 15.02.2026 --- 		debug_str_l += ";my_slope_long_the_best"
			//--- 15.02.2026 --- 	/*||
			//--- 15.02.2026 --- 		slope_long = div_slope_long << div_slope_long > slope_long & div_slope_long > my_slope_long;*/
			//--- 15.02.2026 --- 		// Debug
			//--- 15.02.2026 --- 		//debug_str_l += ";div_slope_long_the_best"
			//--- 15.02.2026 --- 	||
			//--- 15.02.2026 --- 		slope_long = slope_long << /*slope_long >= div_slope_long  &*/ slope_long >= my_slope_long;
			//--- 15.02.2026 --- 		// Debug
			//--- 15.02.2026 --- 		debug_str_l += ";slope_long_the_best"
			//--- 15.02.2026 --- 	};
			//--- 15.02.2026 --- 
			//--- 15.02.2026 --- 	// Debug
			//--- 15.02.2026 --- 	//log(debug_str_l + ";selected...");
			//--- 15.02.2026 --- 	old_nextTSlong = nextTSlong;
			//--- 15.02.2026 --- 
			//--- 15.02.2026 --- 	calc_nextTSlong = (nextTSlong + 1p * slope_long);
			//--- 15.02.2026 --- 
			//--- 15.02.2026 --- 	{
			//--- 15.02.2026 --- 		nextTSlong = my_nextTSlong << my_nextTSlong >= calc_nextTSlong;
			//--- 15.02.2026 --- 		// Debug
			//--- 15.02.2026 --- 		debug_str_l += ";my_nextTSlong_the_best";
			//--- 15.02.2026 --- 	||
			//--- 15.02.2026 --- 		nextTSlong = calc_nextTSlong << calc_nextTSlong > my_nextTSlong;
			//--- 15.02.2026 --- 		// Debug
			//--- 15.02.2026 --- 		debug_str_l += ";calc_nextTSlong_the_best";
			//--- 15.02.2026 --- 	};
			//--- 15.02.2026 --- 		
			//--- 15.02.2026 --- 	// Debug
			//--- 15.02.2026 --- 	log(debug_str_l + ";my_nextTSlong=;" + my_nextTSlong + ";old_nextTSlong=;" + old_nextTSlong + ";nextTSlong=;" + nextTSlong
			//--- 15.02.2026 --- 		+ ";c_nextTSlong=;" + calc_nextTSlong
			//--- 15.02.2026 --- 		+ ";my_slope_long=;" + my_slope_long + ";old_slope_long=;" + old_slope_long + ";slope_long=;" + slope_long
			//--- 15.02.2026 --- 		//+ ";div_slope_long=;" + div_slope_long
			//--- 15.02.2026 --- 	);
			//--- 15.02.2026 --- 
			//--- 15.02.2026 --- ||
			//--- 15.02.2026 --- 	slope_long = slope_long << account <= 0l;
			//--- 15.02.2026 --- 	//nextTSlong = nextTSlong;
			//--- 15.02.2026 --- 	
			//--- 15.02.2026 --- 	// Debug
			//--- 15.02.2026 --- 	log("debug_moving_nextTSlong;skip");
			//--- 15.02.2026 --- };
		...
		&&
			//--- 14.02.2026 --- {
			//--- 14.02.2026 --- 	// Debug
			//--- 14.02.2026 --- 	//old_slope_short = slope_short;
			//--- 14.02.2026 --- 	//debug_str_s = "debug_moving_nextTSshort";
			//--- 14.02.2026 --- 
			//--- 14.02.2026 --- 	//log(debug_str_s + ";started...");
			//--- 14.02.2026 --- 
			//--- 14.02.2026 --- 	my_slope_short = ind("LinearRegression", "slope", "high", predict_window, low_offset, train_window)[-1c] << account < 0l;
			//--- 14.02.2026 --- 	my_nextTSshort = (ind("LinearRegression", "low", "high", predict_window, high_offset, train_window)[-1c]);
			//--- 14.02.2026 --- 	//div_slope_short = ((my_nextTSshort - ind("LinearRegression", "low", "high", predict_window, low_offset, train_window)[-2c]) / 1p);
			//--- 14.02.2026 --- 	{
			//--- 14.02.2026 --- 		slope_short = my_slope_short << my_slope_short < slope_short /*& my_slope_short < div_slope_short*/;
			//--- 14.02.2026 --- 		// Debug
			//--- 14.02.2026 --- 		//debug_str_s += ";my_slope_short_the_best"
			//--- 14.02.2026 --- 	/*||
			//--- 14.02.2026 --- 		slope_short = div_slope_short << div_slope_short < slope_short & div_slope_short < my_slope_short;*/
			//--- 14.02.2026 --- 		// Debug
			//--- 14.02.2026 --- 		//debug_str_s += ";div_slope_short_the_best"
			//--- 14.02.2026 --- 	||
			//--- 14.02.2026 --- 		slope_short = slope_short << /*slope_short <= div_slope_short  &*/ slope_short <= my_slope_short;
			//--- 14.02.2026 --- 		// Debug
			//--- 14.02.2026 --- 		//debug_str_s += ";slope_short_the_best"
			//--- 14.02.2026 --- 	};
			//--- 14.02.2026 --- 
			//--- 14.02.2026 --- 	// Debug
			//--- 14.02.2026 --- 	//log(debug_str_s + ";selected...");
			//--- 14.02.2026 --- 	//old_nextTSshort = nextTSshort;
			//--- 14.02.2026 --- 
			//--- 14.02.2026 --- 	calc_nextTSshort = (nextTSshort + 1p * slope_short);
			//--- 14.02.2026 --- 	
			//--- 14.02.2026 --- 	{
			//--- 14.02.2026 --- 		nextTSshort = my_nextTSshort << my_nextTSshort <= calc_nextTSshort;
			//--- 14.02.2026 --- 		// Debug
			//--- 14.02.2026 --- 		//debug_str_s += ";my_nextTSshort_the_best";
			//--- 14.02.2026 --- 	||
			//--- 14.02.2026 --- 		nextTSshort = calc_nextTSshort << calc_nextTSshort < my_nextTSshort;
			//--- 14.02.2026 --- 		// Debug
			//--- 14.02.2026 --- 		//debug_str_s += ";calc_nextTSshort_the_best";
			//--- 14.02.2026 --- 	};
			//--- 14.02.2026 --- 		
			//--- 14.02.2026 --- 	// Debug
			//--- 14.02.2026 --- 	//log(debug_str_s + ";my_nextTSshort=;" + my_nextTSshort + ";old_nextTSshort=;" + old_nextTSshort + ";nextTSshort=;" + nextTSshort
			//--- 14.02.2026 --- 	//	+ ";c_nextTSshort=;" + calc_nextTSshort
			//--- 14.02.2026 --- 	//	+ ";my_slope_short=;" + my_slope_short + ";old_slope_short=;" + old_slope_short + ";slope_short=;" + slope_short
			//--- 14.02.2026 --- 	//	+ ";div_slope_short=;" + div_slope_short
			//--- 14.02.2026 --- 	//);
			//--- 14.02.2026 --- ||
			//--- 14.02.2026 --- 	slope_short = slope_short << account >= 0l;
			//--- 14.02.2026 --- 	nextTSshort = nextTSshort;
			//--- 14.02.2026 --- };
	...
	||
		...
		..[time < expiration_time]
		{
			...
			||
				...
				LRSLAL_long_CalcNextTSSlope() << my_account > 0l;
				
				//nextTSlong = find_min_price(train_window) << my_account > 0l;
				//slope_long = slope_long_start;
				
				SL_rate_long = CalculateSLLong(safety_stock, risk_L);
				absSLlong = (pos.price + CalculateSLLong(safety_stock, risk_L));
				log("account_>_0l_already;pos.price_=;" + pos.price + ";account_=;" + account 
					+ ";nextTSlong=;" + nextTSlong + ";slope_long=;" + slope_long 
					+ ";absSLlong=;" + absSLlong
			...
			||
				...
				//log("account_<_0l_already_started...");
				// ---
				LRSLAL_short_CalcNextTSSlope() << my_account < 0l;
				
				//nextTSshort = find_max_price(train_window) << my_account < 0l;
				//slope_short = slope_short_start;
				
				SL_rate_short = CalculateSLShort(safety_stock, risk_S);
				absSLshort = (pos.price + CalculateSLShort(safety_stock, risk_S));
				log("account_<_0l_already;pos.price_=;" + pos.price + ";account_=;" + account + ";nextTSshort=;" + nextTSshort + ";slope_short=;" + slope_short 
					+ ";absSLshort=;" + absSLshort
