...
) :=
{
	...
	_long_con0 = ((time < expiration_time & (time >= _day_start_time & time < _day_end_time | time >= _night_start_time & time < _night_end_time) & account == 0l);
	{
		...
		
		{
			...
			{
				_long_con16 = (close[offset] > ind("LinearRegression", "high", B_low_price_type, B_predict_window_type, B_low_offset_type, B_train_window_period)) << _long_con12 == true;
				
				{
					...
					_long_con19 = (OBVP(long_open_OBV_period, long_open_OBV_period/*B_train_window_period*/) > long_open_OBV_level);
		...
		
		{
			...
			
			{
				...
				_long_con79 = (OBVP(long_open_OBV_period, long_open_OBV_period/*B_train_window_period*/) > long_open_OBV_level);
...
) :=
{
	...
	_short_con0 = ((time < expiration_time & (time >= _day_start_time & time < _day_end_time | time >= _night_start_time & time < _night_end_time) & account == 0l);
	{
		...
		_short_con7 = (close[offset] #_ ind("LinearRegression", short_open_B_line, B_low_price_type, B_predict_window_type, B_low_offset_type, B_train_window_period)[offset]) << _short_con0 == true;
		{
			...
			{
				_short_con16 = (close[offset] < ind("LinearRegression", "low", B_high_price_type, B_predict_window_type, B_high_offset_type, B_train_window_period)) << _short_con12 == true;
						
				{
					...
					_short_con19 = (OBVP(short_open_OBV_period, short_open_OBV_period/*B_train_window_period*/) < short_open_OBV_level);
