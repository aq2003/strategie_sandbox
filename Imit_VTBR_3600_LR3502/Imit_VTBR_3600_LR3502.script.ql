...
safety_stock = 10%;	// Safety stock in percents to the equity
...
//  B_channel:
B_train_window_period = 1620c; // a number of candles in history to train the model on
B_predict_window_type = "day"; // a period of time in the future for forecasting := ("candle" || "day" || "week")
...
B_high_offset_type = "none"; //:= ("high" || "low" || "none")
B_low_offset_type = "none"; //:= ("high" || "low" || "none")
...
short_open_OBV_period = 5c;
short_open_OBV_level = 0%; // level of OBV for condition9
	
//  long_closing:
longTS_line = "low";//"high"; // a line type of the indicator which serves as nextTSlong base
longTS_price_type = "high";//A_low_price_type; // a price type of the indicator which serves as nextTSlong base
...
longTP_price_type = B_high_price_type; // a price type of the indicator which serves as trailing TP base
longTP_offset_type = B_high_offset_type; // an offset type of the indicator which serves as trailing TP base
longTP_train_window_period = B_train_window_period; // a train_window period of the indicator which serves as trailing TP base
longTP_predict_window_type = B_predict_window_type; // a predict_window type of the indicator which serves as trailing TP base
longTP_level = 12%; // TP level in percents
	
//  short_closing:
shortTS_line = "high";//"low"; // a line type of the indicator which serves as nextTSshort base
...
shortTP_price_type = B_low_price_type; // a price type of the indicator which serves as trailing TP base
shortTP_offset_type = B_low_offset_type; // an offset type of the indicator which serves as trailing TP base
shortTP_train_window_period = B_train_window_period; // a train_window period of the indicator which serves as trailing TP base
shortTP_predict_window_type = B_predict_window_type; // a predict_window type of the indicator which serves as trailing TP base
shortTP_level = 12%; // TP level in percents

longTS_slope_start_max = (close * 0.5% / 1p); // Starting max slope of linear regression for a long position
shortTS_slope_start_max = -(close * 0.5% / 1p); // Starting max slope of linear regression for a short position

// --- parameters -----------------------------------------------------------------------------------------
		
import("%QTrader_Libs%\LR_strategy_SlopeLevel_AdaptiveLots (35-022).aql");
