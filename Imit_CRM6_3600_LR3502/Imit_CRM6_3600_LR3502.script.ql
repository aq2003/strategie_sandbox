...
//04.04.2026 11:47	TestGrid_CRM6_3600_LR3502.76	Warning	#Test_stopped, , equity=100 000,00p, account=0l, best_equity=166 269,00p, best_max_equity=191 797,50p, best_min_equity=94 060,00p, mean_equity=92 382,43p, safety_stock, =5%, risk_L, =100%, risk_S, =100%, expiration_time, =15:00_31.12.26, day_start_time, =07:00_26.11.25, day_end_time, =19:00_26.11.25, night_start_time, =19:10_26.11.25, night_end_time, =23:49_26.11.25, 
...
//  A_channel:
//A_train_window_period, =266c, A_predict_window_type, =candle, A_high_price_type, =high, A_low_price_type, =low, A_high_offset_type, =none, A_low_offset_type, =none, 
A_train_window_period = 266c; // a number of candles in history to train the model on
...
//  B_channel:
//B_train_window_period, =363c, B_predict_window_type, =week, B_high_price_type, =high, B_low_price_type, =low, B_high_offset_type, =high, B_low_offset_type, =low, 
B_train_window_period = 363c; // a number of candles in history to train the model on
...
// long_opening:
//long_open_A_line, =high, long_open_B_line, =high, long_open_OBV_period, =20c, long_open_OBV_level, =0%, 
...
long_open_OBV_level = 0%; // level of OBV for condition9
	
// short_opening:
//short_open_A_line, =low, short_open_B_line, =low, short_open_OBV_period, =20c, short_open_OBV_level, =0%, 
...
short_open_OBV_level = 0%; // level of OBV for condition9
	
// long_closing:
//longTS_line, =high, longTS_price_type, =low, longTS_offset_type, =none, longTS_train_window_period, =266c, longTS_predict_window_type, =candle, 
//longTS_slope_start, =0n, longTP_line, =low, longTP_price_type, =high, longTP_offset_type, =high, longTP_train_window_period, =363c, longTP_predict_window_type, =week, longTP_level, =50%, 
longTS_line = "high"; // a line type of the indicator which serves as nextTSlong base
longTS_price_type = "low";//A_low_price_type; // a price type of the indicator which serves as nextTSlong base
longTS_offset_type = "none";//A_low_offset_type; // an offset type of the indicator which serves as nextTSlong base
longTS_train_window_period = 266c;//A_train_window_period; // a train_window period of the indicator which serves as nextTSlong base	
longTS_predict_window_type = "candle";//A_predict_window_type; // a predict_window type of the indicator which serves as nextTSlong base	
longTS_slope_start = 0n;//(close * 0.01% / 1p); // Starting slope of linear regression for a long position
longTP_line = "low"; // a line type of the indicator which serves as trailing TP base
longTP_price_type = "high";//B_high_price_type; // a price type of the indicator which serves as trailing TP base
longTP_offset_type = "high";//B_high_offset_type; // an offset type of the indicator which serves as trailing TP base
longTP_train_window_period = 363c;//B_train_window_period; // a train_window period of the indicator which serves as trailing TP base
longTP_predict_window_type = "week";//B_predict_window_type; // a predict_window type of the indicator which serves as trailing TP base
longTP_level = 50%; // TP level in percents
	
// short_closing:
//shortTS_line, =low, shortTS_price_type, =low, shortTS_offset_type, =none, shortTS_train_window_period, =266c, shortTS_predict_window_type, =candle, 
//shortTS_slope_start, =0n, shortTP_line, =high, shortTP_price_type, =low, shortTP_offset_type, =low, shortTP_train_window_period, =363c, shortTP_predict_window_type, =week, shortTP_level, =50%      
shortTS_line = "low"; // a line type of the indicator which serves as nextTSshort base
shortTS_price_type = "low"; //A_high_price_type; // a price type of the indicator which serves as nextTSshort base
shortTS_offset_type = "none";//A_high_offset_type; // an offset type of the indicator which serves as nextTSshort base
shortTS_train_window_period = 266c;//A_train_window_period; // a train_window period of the indicator which serves as nextTSshort base	
shortTS_predict_window_type = "candle";//A_predict_window_type; // a predict_window type of the indicator which serves as nextTSshort base	
shortTS_slope_start = 0n;//-(close * 0.01% / 1p); // Starting slope of linear regression for a short position
shortTP_line = "high"; // a line type of the indicator which serves as trailing TP base
shortTP_price_type = "low";//B_low_price_type; // a price type of the indicator which serves as trailing TP base
shortTP_offset_type = "low";//B_low_offset_type; // an offset type of the indicator which serves as trailing TP base
shortTP_train_window_period = 363c;//B_train_window_period; // a train_window period of the indicator which serves as trailing TP base
shortTP_predict_window_type = "week";//B_predict_window_type; // a predict_window type of the indicator which serves as trailing TP base
shortTP_level = 50%; // TP level in percents

// --- parameters -----------------------------------------------------------------------------------------
		
import("%QTrader_Libs%\LR_strategy_SlopeLevel_AdaptiveLots (35-021).aql");
