// 12.10.2021 15:54:49 VTBR_900_LR ql script
// Created 12.10.2021 15:54:49
...
train_window_support = 342c;
predict_window_resistance = "week";
train_window_resistance = 342c;
// --- parameters -----------------------------------------------------------------------------------------
	...
	..[time < expiration_time]
	{
		my_account = account;
		{
			long(lots) << time < expiration_time & account == 0l
				...
				& ind("LinearRegression", "high", "high", predict_window, low_offset, train_window) 
					<= ind("LinearRegression", "high", "high", predict_window, low_offset, train_window)[-1c]
				& (
				...
				//	- ind("LinearRegression", "high", "low", predict_window_support, "low", train_window_support)) > 950p
			;
		...
		||
			short(lots) << time < expiration_time & account == 0l
				...
				& ind("LinearRegression", "low", "low", predict_window, low_offset, train_window) 
					>= ind("LinearRegression", "low", "low", predict_window, low_offset, train_window)[-1c]
				& close[-1c] < (LR_sup = ind("LinearRegression", "low", "high", predict_window_resistance, "high", train_window_resistance)[-1c])
				& (
					close[-1c] > (LR_sup = ind("LinearRegression", "high", "low", predict_window_support, "low", train_window_support)[-1c])
